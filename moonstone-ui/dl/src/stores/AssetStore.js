...
class AssetStore extends BaseStore {
    ...
    onGetAssetList(payload) {
        ...
        payload.assets.forEach(asset => {
            ...
            if (asset.smartasset_data_id) {
                ...
                for (var i = 0; i < payload.smartasset_data.length; i++) {
                    if (payload.smartasset_data[i].id === asset.smartasset_data_id) {
                        asset.smartasset_data = payload.smartasset_data[i];
                        break;
                    }
                }
            } else {
    ...
    onGetAsset(payload) {
        ...
        if (payload.smartasset_data) {
            asset.smartasset_data = payload.smartasset_data;
            asset.market_asset = true;
        } else {
            asset.market_asset = false;
        }
