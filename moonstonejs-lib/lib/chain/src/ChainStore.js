...
var {Apis} = require("moonstonejs-ws");
...
let smartasset_data_prefix = "2." + parseInt(impl_object_type.asset_smartasset_data,10) + "."
...
const DEBUG = JSON.parse(process.env.npm_config__moonstone_chain_chain_debug || false)
...
class ChainStore
{
   ...
   getAsset( id_or_symbol )
   {
      ...
      if ( ChainValidation.is_object_id( id_or_symbol ) ) {
        ...
        if (asset && (asset.get("smartasset") && !asset.getIn(["smartasset", "current_feed"]))) {
          return undefined;
        }
        return asset;
      }
   ...
   _updateObject( object, notify_subscribers, emit = true )
   {
      ...
      else if( object.id.substring(0,asset_prefix.length) == asset_prefix )
      {
         ...
         let smartasset = current.get( 'smartasset' );
         if( !smartasset && object.smartasset_data_id ) {
            let bad = this.getObject( object.smartasset_data_id, true );
            if( !bad )
               bad = Immutable.Map()
            ...
            this.objects_by_id = this.objects_by_id.set( object.smartasset_data_id, bad );

            current = current.set( 'smartasset', bad );
            this.objects_by_id = this.objects_by_id.set( object.id, current );
         }
      }
      ...
      else if( object.id.substring(0,smartasset_data_prefix.length) == smartasset_data_prefix )
      {
          let asset_id = current.get( "asset_id" );
          if( asset_id ) {
             let asset = this.getObject( asset_id );
             if( asset ) {
                asset = asset.set( "smartasset", current );
                emitter.emit('smartasset-update', asset);
