...
class AssetName extends React.Component {
	...
	render() {
		...
		let isSmartAsset = asset.has("smartasset");
		let isPredMarket = isSmartAsset && asset.getIn(["smartasset", "is_prediction_market"]);

		let {name: replacedName, prefix} = utils.replaceName(name, isSmartAsset && !isPredMarket && asset.get("issuer") === "1.2.0");
		// let prefix = isSmartAsset && !isPredMarket ? <span>bit</span> :
