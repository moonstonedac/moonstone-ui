...
class Asset extends React.Component {
    ...
    _assetType(asset) {
        return ('smartasset' in asset) ?
               (asset.smartasset.is_prediction_market ? 'Prediction' : 'Smart') :
               'Simple';
    }
    ...
    renderAboutBox(asset) {
        ...
        let {name, prefix} = utils.replaceName(asset.symbol, "smartasset" in asset && !asset.smartasset.is_prediction_market && asset.issuer === "1.2.0");
    ...
    renderSummary(asset) {
        ...
        let flagBooleans = assetUtils.getFlagBooleans(asset.options.flags, this.props.asset.has("smartasset_data_id"));
    ...
    renderPriceFeed(asset) {
        ...
        var smartAsset = asset.smartasset;
        if (!('current_feed' in smartAsset))
            return ( <div header= {title} /> );
        var currentFeed = smartAsset.current_feed;
    ...
    renderPermissions(asset) {
        ...
        let permissionBooleans = assetUtils.getFlagBooleans(asset.options.issuer_permissions, this.props.asset.has("smartasset_data_id"));
    ...
    renderPriceFeedData(asset) {
        ...
        var smartAsset = asset.smartasset;
        if (!('feeds' in smartAsset) || smartAsset.feeds.length == 0 || smartAsset.is_prediction_market) {
            return null;
        }

        let now = new Date().getTime();
        let oldestValidDate = new Date(now - asset.smartasset.options.feed_lifetime_sec * 1000);

        // Filter by valid feed lifetime, Sort by published date
        var feeds = smartAsset.feeds;
        feeds = feeds
        .filter(a => {
    ...
    render()
    {
        ...
        var priceFeed = ('smartasset' in asset) ? this.renderPriceFeed(asset) : null;
        var priceFeedData = ('smartasset' in asset) ? this.renderPriceFeedData(asset) : null;
