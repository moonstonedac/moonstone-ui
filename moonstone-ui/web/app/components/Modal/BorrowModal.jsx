 ...
 *
 *  Expected Properties:
 *     quote_asset:  asset id, must be a smartasset
...
class BorrowModalContent extends React.Component {
    ...
    static propTypes = {
        ...
        smartasset_balance: ChainTypes.ChainObject,
    ...
    _validateFields(newState) {
        ...
        let sqp = this.props.quote_asset.getIn(["smartasset", "current_feed", "maximum_short_squeeze_ratio"]) / 1000;
        let mcr = this.props.quote_asset.getIn(["smartasset", "current_feed", "maintenance_collateral_ratio"]) / 1000
    ...
    _getFeedPrice() {
        ...
        return 1 / utils.get_asset_price(
            this.props.quote_asset.getIn(["smartasset", "current_feed", "settlement_price", "quote", "amount"]),
            this.props.backing_asset,
            this.props.quote_asset.getIn(["smartasset", "current_feed", "settlement_price", "base", "amount"]),
    ...
    _isPredictionMarket(props) {
        return props.quote_asset.getIn(["smartasset", "is_prediction_market"]);
    }


    _calculateCallPrice() {
        let CALL_PRICE = 0;
        let maintenanceRatio = this.props.quote_asset.getIn(["smartasset", "current_feed", "maintenance_collateral_ratio"]) / 1000;
    ...
    render() {
        let {quote_asset, smartasset_balance, backing_asset, backing_balance} = this.props;
        ...
        smartasset_balance = !smartasset_balance ? {balance: 0, id: null} : smartasset_balance.toJS();
        ...
        let smartAssetBalanceText = <span><Translate component="span" content="transfer.available"/>: {smartasset_balance.id ? <BalanceComponent balance={smartasset_balance.id}/> : <FormattedAsset amount={0} asset={quote_asset.get("id")} />}</span>;
        ...
        let maintenanceRatio = this.props.quote_asset.getIn(["smartasset", "current_feed", "maintenance_collateral_ratio"]) / 1000;
        let squeezeRatio = this.props.quote_asset.getIn(["smartasset", "current_feed", "maximum_short_squeeze_ratio"]) / 1000;
                ...
                <form className="grid-container small-10 small-offset-1 no-overflow" noValidate>
                        ...
                        <div style={{paddingBottom: "1rem"}} >
                            <div className="borrow-price-feeds">
                                ...
                                <FormattedPrice
                                    ...
                                    quote_amount={quote_asset.getIn(["smartasset", "current_feed", "settlement_price", "base", "amount"])}
                                    quote_asset={quote_asset.getIn(["smartasset", "current_feed", "settlement_price", "base", "asset_id"])}
                                    base_asset={quote_asset.getIn(["smartasset", "current_feed", "settlement_price", "quote", "asset_id"])}
                                    base_amount={quote_asset.getIn(["smartasset", "current_feed", "settlement_price", "quote", "amount"])}
                                    />
                            </div>
                            <div className="borrow-price-feeds">
                                ...
                                <FormattedPrice
                                    ...
                                    quote_asset={quote_asset.getIn(["smartasset", "current_feed", "settlement_price", "base", "asset_id"])}
                                    base_asset={quote_asset.getIn(["smartasset", "current_feed", "settlement_price", "quote", "asset_id"])}
                                    base_amount={squeezeRatio * quote_asset.getIn(["smartasset", "current_feed", "settlement_price", "quote", "amount"])}
                                    />
                            </div>
                            <b/>
                    ...
                    <div className="form-group">
                        <AmountSelector label="transaction.borrow_amount"
                                        ...
                                        display_balance={smartAssetBalanceText}
                                        placeholder="0.0"
                                        tabIndex={1}/>
                    </div>
...
export default class ModalWrapper extends React.Component {
    ...
    render() {
        ...
        let coreBalance, smartAssetBalance;
        ...
        if (accountBalance) {
            for (var id in accountBalance) {
                ...
                if (id === quote_asset) {
                    smartAssetBalance = accountBalance[id];
                }
            }
        }
            ...
            <Modal id={modalId} overlay={true} ref={modalId}>
                ...
                <div className="grid-block vertical">
                    <BorrowModalContent
                        ...
                        modalId={modalId}
                        smartasset_balance={smartAssetBalance}
