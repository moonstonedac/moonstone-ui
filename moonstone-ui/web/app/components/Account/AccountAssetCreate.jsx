...
import {ChainStore, ChainValidation} from "moonstonejs-lib";
...
@BindToChainState()
class SmartAssetOptions extends React.Component {
    ...
    _onFoundBackingAsset(asset) {
        if (asset) {
            if (asset.get("id") === "1.3.0" || (asset.get("smartasset_data_id") && !asset.getIn(["smartasset", "is_prediction_market"]))) {       
    ...
    render() {
        let {smartasset_opts} = this.props;
        let {error} = this.state;

        return (
            <div>
                <label><Translate content="account.user_issued_assets.feed_lifetime_sec" />
                    <input type="number" value={smartasset_opts.feed_lifetime_sec / 60} onChange={this.props.onUpdate.bind(this, "feed_lifetime_sec")}/>
                </label>

                <label><Translate content="account.user_issued_assets.minimum_feeds" />
                    <input type="number" value={smartasset_opts.minimum_feeds} onChange={this.props.onUpdate.bind(this, "minimum_feeds")}/>
                </label>

                <label><Translate content="account.user_issued_assets.force_settlement_delay_sec" />
                    <input type="number" value={smartasset_opts.force_settlement_delay_sec / 60} onChange={this.props.onUpdate.bind(this, "force_settlement_delay_sec")}/>
                </label>

                <label><Translate content="account.user_issued_assets.force_settlement_offset_percent" />
                    <input type="number" value={smartasset_opts.force_settlement_offset_percent / assetConstants.MOONSTONE_1_PERCENT} onChange={this.props.onUpdate.bind(this, "force_settlement_offset_percent")}/>
                </label>

                <label><Translate content="account.user_issued_assets.maximum_force_settlement_volume" />
                    <input type="number" value={smartasset_opts.maximum_force_settlement_volume / assetConstants.MOONSTONE_1_PERCENT} onChange={this.props.onUpdate.bind(this, "maximum_force_settlement_volume")}/>
                </label>
...
class AccountAssetCreate extends React.Component {
    ...
    resetState(props) {
        ...
        let isSmartAsset = false;
        let precision = utils.get_asset_precision(4);
        let corePrecision = utils.get_asset_precision(props.core.get("precision"));

        let {flagBooleans, permissionBooleans} = this._getPermissions({isSmartAsset});
        
        // let flags = assetUtils.getFlags(flagBooleans);
        // let permissions = assetUtils.getPermissions(permissionBooleans, isSmartAsset);
        ...
        return {
            ...
            smartasset_opts: {
                ...
                "force_settlement_offset_percent" : 1 * assetConstants.MOONSTONE_1_PERCENT,
                "maximum_force_settlement_volume" : 20 * assetConstants.MOONSTONE_1_PERCENT,
    ...
    _getPermissions(state) {
        let flagBooleans = assetUtils.getFlagBooleans(0, state.isSmartAsset);
        let permissionBooleans = assetUtils.getFlagBooleans("all", state.isSmartAsset);
    ...
    _createAsset(e) {
        ...
        let {update, flagBooleans, permissionBooleans, core_exchange_rate,
            isSmartAsset, is_prediction_market, smartasset_opts} = this.state;
        
        let {account} = this.props;

        let flags = assetUtils.getFlags(flagBooleans, isSmartAsset);
        let permissions = assetUtils.getPermissions(permissionBooleans, isSmartAsset);
        ...
        AssetActions.createAsset(account.get("id"), update, flags, permissions, core_exchange_rate, isSmartAsset, is_prediction_market, smartasset_opts, description).then(result => {
    ...
    onChangeSmartAssetOpts(value, e) {
        let {smartasset_opts} = this.state;

        switch (value) {
            case "force_settlement_offset_percent":
            case "maximum_force_settlement_volume":
                smartasset_opts[value] = parseFloat(e.target.value) * assetConstants.MOONSTONE_1_PERCENT;
                break;

            case "feed_lifetime_sec":
            case "force_settlement_delay_sec":
                console.log(e.target.value, parseInt(parseFloat(e.target.value) * 60, 10));
                smartasset_opts[value] = parseInt(parseFloat(e.target.value) * 60, 10);
                break;

            case "short_backing_asset":
                smartasset_opts[value] = e;
                break;

            default:
                smartasset_opts[value] = e.target.value;
                break;
        }
    ...
    _onToggleSmartAsset() {
        let {update} = this.state;
        this.state.isSmartAsset = !this.state.isSmartAsset;
        if (!this.state.isSmartAsset) {
    ...
    render() {
        ...
        let {errors, isValid, update, assets, flagBooleans, permissionBooleans,
            core_exchange_rate, is_prediction_market, isSmartAsset, smartasset_opts} = this.state;
            ...
            <div className="grid-block">
                <div className="grid-content">
                    ...
                    <Tabs setting="createAssetTab" style={{maxWidth: "800px"}} contentClass="grid-block shrink small-vertical medium-horizontal">
                                ...
                                <table className="table" style={{width: "inherit"}}>
                                    <tbody>
                                        <tr>
                                            ...
                                            <td style={{border: "none"}}>
                                                <div className="switch" style={{marginBottom: "10px"}} onClick={this._onToggleSmartAsset.bind(this)}>
                                                    <input type="checkbox" checked={isSmartAsset} />
                                                    <label />
                                                </div>
                                            </td>
                                        </tr>
                        ...
                        {isSmartAsset ? (
                            <Tab title="account.user_issued_assets.smartasset_opts">
                                <SmartAssetOptions
                                    smartasset_opts={smartasset_opts}
                                    onUpdate={this.onChangeSmartAssetOpts.bind(this)}
                                    backingAsset={smartasset_opts.short_backing_asset}
...
AccountAssetCreate.SmartAssetOptions = SmartAssetOptions;
