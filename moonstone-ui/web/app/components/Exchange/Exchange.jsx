...
import {ChainStore, EmitterInstance} from "moonstonejs-lib";
...
class Exchange extends React.Component {
    ...
    componentWillMount() {
        ...
        emitter.on('smartasset-update', feedUpdateListener = MarketsActions.feedUpdate);
    ...
    componentWillReceiveProps(nextProps) {
        if (nextProps.baseAsset && nextProps.baseAsset.getIn(["smartasset", "is_prediction_market"])) {
    ...
    componentWillUnmount() {
        ...
        if (emitter) {
            ...
            emitter.off('smartasset-update', feedUpdateListener);
    }
    ...
    _createLimitOrderConfirm(buyAsset, sellAsset, buyAssetAmount, sellAssetAmount, sellBalance, coreBalance, feeAsset, type, short = true, e) {
        ...
        let isPredictionMarket = sellAsset.getIn(["smartasset", "is_prediction_market"]);
    ...
    _getSettlementInfo() {
        ...
        if (isMarketAsset && quote && base) {
            if (quote.get("smartasset") && quote.getIn(["smartasset", "current_feed"]) && base.get("id") === quote.getIn(["smartasset", "options", "short_backing_asset"])) {
                settlement_price = quote.getIn(["smartasset", "current_feed", "settlement_price"]);
                short_squeeze = quote.getIn(["smartasset", "current_feed", "maximum_short_squeeze_ratio"]) / 1000;

            } else if (base.get("smartasset") && base.getIn(["smartasset", "current_feed"]) && quote.get("id") === base.getIn(["smartasset", "options", "short_backing_asset"])) {
                settlement_price = base.getIn(["smartasset", "current_feed", "settlement_price"]);
                short_squeeze = base.getIn(["smartasset", "current_feed", "maximum_short_squeeze_ratio"]) / 1000;
            }
    ...
    render() {
        ...
        let quoteIsSmartAsset = quoteAsset.get("smartasset_data_id") ? true : false;
        let baseIsSmartAsset = baseAsset.get("smartasset_data_id") ? true : false;
        ...
        let hasPrediction = base.getIn(["smartasset", "is_prediction_market"]) || quote.getIn(["smartasset", "is_prediction_market"]);
        ...
        let buyForm = (
            <BuySell
                ...
                isPredictionMarket={base.getIn(["smartasset", "is_prediction_market"])}
        ...
        let sellForm = (
            <BuySell
                ...
                isPredictionMarket={quote.getIn(["smartasset", "is_prediction_market"])}
                onFlip={this.state.flipBuySell ? this._flipBuySell.bind(this) : null}
            />
        );
                ...
                <div className="grid-block page-layout market-layout">
                                ...
                                <div className="grid-block vertical">
                                    ...
                                    <div className="grid-block wrap no-overflow" style={{justifyContent: "space-between"}}>
                                        <ul className="market-stats stats bottom-stats">
                                         ...
                                         <ul className="market-stats stats bottom-stats">
                                            {!isNullAccount && quoteIsSmartAsset ?
                                            ...
                                            {!isNullAccount && baseIsSmartAsset ?
                    ...
                    {!isNullAccount && quoteIsSmartAsset  ?
                        <BorrowModal
                            ...
                            backing_asset={quoteAsset.getIn(["smartasset", "options", "short_backing_asset"])}
                    ...
                    {!isNullAccount && baseIsSmartAsset ?
                        <BorrowModal
                            ...
                            backing_asset={baseAsset.getIn(["smartasset", "options", "short_backing_asset"])}
                            account={currentAccount}
                        /> : null}
