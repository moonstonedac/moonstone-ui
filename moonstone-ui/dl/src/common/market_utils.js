...
import {ChainStore, ChainTypes} from "moonstonejs-lib";
...
class MarketUtils {
    ...
    static isMarketAsset(quote, base) {
        ...
        if (quote.get("smartasset") && base.get("id") === quote.getIn(["smartasset", "options", "short_backing_asset"])) {
            isMarketAsset = true;
            marketAsset = {id: quote.get("id")}
        } else if (base.get("smartasset") && quote.get("id") === base.getIn(["smartasset", "options", "short_backing_asset"])) {
            inverted = true;
            isMarketAsset = true;
